...
/** UdpMultiRelay implements an UDP multiple relay agent. 
  * It receives UDP packets at a org.local socket and relays them
...
public class UdpMultiRelay extends Thread {
  ...
  /** Whether sending all packets to a remote node using the same socket where the remote node sends its packet to (per-destinaiotn outgoing socket). */
	...
	/** Whether filtering packet directed to org.local socket's remote address */
	boolean filtered;
	...
	/** Creates a new UDP relay and starts it.
	  * @param socket org.local socket where packet arrives (and where are sent from, in case of per-destination outgoing socket)
	  ...
	  * @param filtered whether filtering packet directed to org.local socket's remote address
	...
	/** Inits a new UDP relay and starts it.
	  * @param local_port org.local port used for both incoming and outgoing packet fows
