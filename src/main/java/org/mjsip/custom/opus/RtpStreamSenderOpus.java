...
/** RtpStreamSender is a generic RTP sender.
  ...
  */
public class RtpStreamSenderOpus extends Thread implements RtpControlledSender {
	...
	/** The InputStream */
	OpusInputStream input_stream=null;
	...
	/** Whether it works synchronously with a org.local clock, or it it acts as slave of the InputStream  */
	...
	public RtpStreamSenderOpus(OpusInputStream input_stream, boolean do_sync, int payload_type, long sample_rate, int channels, long packet_time, int payload_size, Encoder additional_encoder, String dest_addr, int dest_port, RtpStreamSenderListener listener) throws java.net.SocketException, java.net.UnknownHostException {
	...
	/** Constructs a RtpStreamSender.
	  ...
	  * @param dest_addr the destination address
	  * @param dest_port the destination port */
	public RtpStreamSenderOpus(OpusOutputStream outputStream, OpusInputStream input_stream, boolean do_sync, int payload_type, long sample_rate, int channels, long packet_time, int payload_size, Encoder additional_encoder, UdpSocket src_socket, String dest_addr, int dest_port, RtpStreamSenderListener listener) throws java.net.UnknownHostException {
		this.outputStream=outputStream;
		init(input_stream,do_sync,payload_type,sample_rate,channels,packet_time,payload_size,additional_encoder,src_socket,dest_addr,dest_port,listener);
	}                

	OpusOutputStream outputStream;

	/** Inits the RtpStreamSender */
	private void init(OpusInputStream input_stream, boolean do_sync, int payload_type, long sample_rate, int channels, long packet_time, int payload_size, Encoder additional_encoder, UdpSocket src_socket, String dest_addr, int dest_port, RtpStreamSenderListener listener) throws java.net.UnknownHostException {
	...
	public void run() {
		...
		try {
			while (running) {
				...
				if (time>=next_report_time) {
					//if (rtp_control!=null) rtp_control.send(new org.local.net.SrRtcpPacket(rtp_packet.getSsrc(),System.currentTimeMillis(),timestamp,packet_count,octect_count));
		...
		// close RtpSocket and org.local UdpSocket
		...
		if (listener!=null) listener.onRtpStreamSenderTerminated(null,error);
	}
