...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	public void run() {
		...
		System.out.println("H264 packetizer started !");
		stats.reset();
		count = 0;
		...
		try {
			...
			System.out.println("Couldn't skip mp4 header :/");
			return;
		}
...
//						report.send(oldtime+duration,(ts/100)*90/10000);
				...
				delay = stats.average();
				//System.out.println("duration: "+duration/1000000+" delay: "+delay/1000000);

			}
		} catch (IOException e) {
		} catch (InterruptedException e) {}

		System.out.println("H264 packetizer stopped !");
	...
	private void send() throws IOException, InterruptedException {
		...
		//System.out.println("- Nal unit length: " + naluLength + " delay: "+delay/1000000+" type: "+type);
		...
		if (naluLength<=MAXPACKETSIZE-rtphl-2) {
			...
			//System.out.println("----- Single NAL unit - len:"+len+" delay: "+delay);
		}
		// Large NAL unit => Split nal unit 
			...
			while (sum < naluLength) {
				...
				//System.out.println("----- FU-A unit, sum:"+sum);
			}
		}
	}
	...
	private void resync() throws IOException {
		...
		System.out.println("Packetizer out of sync ! Let's try to fix that...");
		...
		while (true) {
			...
			if (type == 5 || type == 1) {
				...
				if (naluLength>0 && naluLength<100000) {
					oldtime = System.nanoTime();
					System.out.println("A NAL unit may have been found in the bit stream !");
					break;
				}
			}
