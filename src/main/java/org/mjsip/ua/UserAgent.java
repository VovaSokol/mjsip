...
public class UserAgent extends CallListenerAdapter implements SipProviderListener, ExtendedCallListener, RegistrationClientListener, TimerListener {
	...
	/** Current org.local media descriptions */
	...
	public static int videoRemotePort = 0;
	public static int audioRemotePort = 0;
	public static int videoLocalPort = 0;
	public static int audioLocalPort = 0;
	...
	/** Gets the org.local SDP */
	/*public String getLocalSDP() {
		return session_descriptor.toString();
	}*/  

	/** Sets the org.local SDP */
	...
	protected void startMediaSessions() {
		...
		// for each media
		FlowSpec videoFLow = null;
		FlowSpec audioFLow = null;
		for (Enumeration ei=md_list.elements(); ei.hasMoreElements(); ) {
			...
			System.out.println(LogLevel.DEBUG + " Media session: " + md.toString());
			System.out.println(LogLevel.DEBUG + " Media session transport: " + md.getTransport());
			System.out.println(LogLevel.DEBUG + " Media session media: " + md.getMedia());
			System.out.println(LogLevel.DEBUG + " Media session format: " + md.getFormats());
			System.out.println(LogLevel.DEBUG + " Media session format list: " + md.getFormatList());
			String format = null;
			int avp = 0;
			if (md.getFormatList().size() != 0) {
				format=(String)md.getFormatList().elementAt(0);
				avp=Integer.parseInt(format);
			}
			...
			if (local_port!=0 && remote_port!=0 && media_spec!=null) {
				...
				if(flow_spec.getMediaSpec().getType().equals("video")){
					videoFLow = flow_spec;
					UserAgent.videoRemotePort = remote_port;
					UserAgent.videoLocalPort = local_port;
				}else{
					audioFLow = flow_spec;
					UserAgent.audioRemotePort = remote_port;
					UserAgent.audioLocalPort = local_port;
				}
		...
		/*if(videoFLow != null){
			boolean success=media_agent.startMediaSession(videoFLow);
			if (success) {
				media_sessions.addElement("video");
				if (listener!=null) listener.onUaMediaSessionStarted(this,"video",videoFLow.getMediaSpec().getCodec());
			}
		}else{
			boolean success=media_agent.startMediaSession(audioFLow);
			if (success) {
				media_sessions.addElement("audio");
				if (listener!=null) listener.onUaMediaSessionStarted(this,"audio",audioFLow.getMediaSpec().getCodec());
			}
		}*/
	}
	...
	public static AudioClipPlayer getAudioClip(String jar_file, String res_path, String image_file) throws java.io.IOException {
