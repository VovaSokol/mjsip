...
import org.mjsip.custom.opus.OpusAudioStreamer;
...
import org.mjsip.custom.video.NativeVideoStreamer;
...
public class MediaAgent {
	...
	public boolean startMediaSession(FlowSpec flow_spec) {
		...
		if (ua_profile.loopback) {
		    media_streamer=new LoopbackMediaStreamer(flow_spec,logger);
        } else if (flow_spec.getMediaSpec().getType().equals("audio")){
		    media_streamer=newAudioStreamer(flow_spec);
        } else if (flow_spec.getMediaSpec().getType().equals("video")){
		    media_streamer=newVideoStreamer(flow_spec);
        } else if (flow_spec.getMediaSpec().getType().equals("ptt")){
		    media_streamer=newPttStreamer(flow_spec);
        }
	...
	private MediaStreamer newAudioStreamer(FlowSpec audio_flow) {
		...
		if (ua_profile.use_jmf_audio) {
			...
			try {
				...
				Class media_streamer_class=Class.forName("org.local.ext.media.jmf.JmfMediaStreamer");
		...
		if (audio_streamer==null) {
			...
			if (ua_profile.javax_sound_streamer==null) {
				...
				if (audio_flow.getMediaSpec().getCodec().equals("OPUS")){
					audio_streamer=new OpusAudioStreamer(audio_flow,audio_in,audio_out,ua_profile.javax_sound_direct_convertion,null,ua_profile.javax_sound_sync,ua_profile.random_early_drop_rate,ua_profile.symmetric_rtp,logger);
				} else {
	...
	private MediaStreamer newVideoStreamer(FlowSpec video_flow) {
        ...
        int remote_port1=(ua_profile.video_mcast_soaddr!=null)? ua_profile.video_mcast_soaddr.getPort() : UserAgent.audioRemotePort;
        int local_port=(ua_profile.video_mcast_soaddr!=null)? ua_profile.video_mcast_soaddr.getPort() : video_flow.getLocalPort();
        int local_port1=(ua_profile.video_mcast_soaddr!=null)? ua_profile.video_mcast_soaddr.getPort() : UserAgent.audioLocalPort;

		String[] args=new String[]{
				(remote_addr+":"+remote_port),
				(remote_addr+":"+remote_port1),
				remote_addr,
				String.valueOf(remote_port),
		};

        video_streamer=new NativeVideoStreamer(video_flow,args,local_port,remote_port,logger);
	...
	private MediaStreamer newPttStreamer(FlowSpec flow_spec) {
		...
		try {
			Class media_streamer_class=Class.forName("org.local.ext.media.push2talk.Push2TalkApp");
			Class[] param_types={ FlowSpec.class, Logger.class };
