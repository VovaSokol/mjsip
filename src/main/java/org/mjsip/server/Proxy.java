...
public class Proxy extends Registrar {
	...
	/** When a new request is received for the org.local server. */
	...
	/** When a new request message is received for a org.local user */
	...
	public void processRequestToRemoteUA(SipMessage msg) {
		...
		if (!server_profile.is_open_proxy) {
			// check whether the caller or callee is a org.local user
			if (!isResponsibleFor(msg.getFromHeader().getNameAddress().getAddress()) && !isResponsibleFor(msg.getToHeader().getNameAddress().getAddress())) {
				log(LogLevel.INFO,"both caller and callee are not registered with the org.local server: proxy denied.");
