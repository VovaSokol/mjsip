...
public abstract class ServerEngine implements SipProviderListener {
	...
	protected static final String[] LOCATION_SERVICES={"org/local", "ldap" };
	/** List of location service Classes (ordered as in <i>LOCATION_SERVICES</i>) */
	protected static final String[] LOCATION_SERVICE_CLASSES={ "org.local.server.LocationServiceImpl", "org.local.ldap.LdapLocationServiceImpl" };

	/** List of already supported authentication services */
	protected static final String[] AUTHENTICATION_SERVICES={"org/local", "ldap" };
	/** List of authentication service Classes (ordered as in <i>AUTHENTICATION_SERVICES</i>) */
	protected static final String[] AUTHENTICATION_SERVICE_CLASSES={ "org.local.server.AuthenticationServiceImpl", "org.local.ldap.LdapAuthenticationServiceImpl" };
	...
	protected static final String[] AUTHENTICATION_SERVER_CLASSES={ "org.local.server.AuthenticationServerImpl" };
	...
	/** When a new request request is received for the org.local server */
	...
	public ServerEngine(SipProvider provider, ServerProfile profile) {
		...
		if (server_profile.do_authentication || server_profile.do_proxy_authentication) {
			...
			try {
				...
				Class[] parameter_types={ Class.forName("java.lang.String"), Class.forName("org.local.server.AuthenticationService"), Class.forName("org.zoolu.util.LogWriter") };
	...
	public void onReceivedMessage(SipProvider provider, SipMessage msg) {
		...
		if (msg.isRequest()) {
			...
			// removes the org.local Route value, if present
			...
			log(LogLevel.TRACE,"is for org.local doamin? "+((is_for_this_domain)?"yes":"no"));
			...
			log(LogLevel.TRACE,"is from org.local doamin? "+((is_from_this_domain)?"yes":"no"));
			...
			if (is_for_this_domain && (target.isSipURI() && !(new SipURI(target)).hasUserName())) {
				...
				if (is_for_this_domain) {
					log(LogLevel.TRACE,"the request is for a org.local user");
					// process the message
	...
	/** Whether the server is responsible for the given <i>domain</i>
	  * (i.e. the <i>domain</i> is included in the org.local domain names list)
	  * and <i>port</i> (if &gt;0) matches the org.local server port. */
	...
	/** Gets a String of the list of org.local domain names. */
